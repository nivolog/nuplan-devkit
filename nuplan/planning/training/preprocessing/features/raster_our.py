...
@dataclass
class RasterOur(AbstractModelFeature):
    """
    ...
    def to_feature_tensor(self) -> AbstractModelFeature:
        ...
        return RasterOur(data=to_tensor_torchvision(np.asarray(self.data)))

    def to_device(self, device: torch.device) -> RasterOur:
        """Implemented. See interface."""
        validate_type(self.data, torch.Tensor)
        return RasterOur(data=self.data.to(device=device))

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> RasterOur:
        """Implemented. See interface."""
        return RasterOur(data=data["data"])

    def unpack(self) -> List[RasterOur]:
        """Implemented. See interface."""
        return [RasterOur(data[None]) for data in self.data]

    @staticmethod
    def from_feature_tensor(tensor: torch.Tensor) -> RasterOur:
        """Implemented. See interface."""
        array = tensor.numpy()
